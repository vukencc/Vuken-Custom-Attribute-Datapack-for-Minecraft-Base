...
execute store result score @s temp_sapper run data get entity @s SelectedItem.components."minecraft:enchantments"."center_calculation:assistance/sapper"
...
function center_calculation:custom_ench/apply/regenerate/restore_hp with entity @n[type=marker,tag=temp_reg] data
execute if entity @s[nbt={SelectedItem:{components:{"minecraft:enchantments":{"center_calculation:assistance/chain_break":1}}}}] at @s run function center_calculation:custom_ench/assistance/chain_break

scoreboard objectives add temp_eruption dummy
execute store result score @s temp_eruption run data get entity @s SelectedItem.components."minecraft:enchantments"."center_calculation:assistance/eruption" 5
execute store result entity @n[type=marker,tag=temp_reg] data.eruption int 1 run scoreboard players get @s temp_eruption
execute if score @s temp_eruption matches 1.. run function center_calculation:custom_ench/assistance/eruption with entity @n[type=marker,tag=temp_reg] data
