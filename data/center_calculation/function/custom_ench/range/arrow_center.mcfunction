...
summon armor_stand ~ ~ ~ {Invisible:1b,NoGravity:1b,Invulnerable:1b,equipment:{saddle:{id:saddle,components:{enchantments:{"center_calculation:inter/arrow_center":1}},count:1}},data:{UUID:[0,0,0,0],starlit_damage:0.0d},Tags:["arrow_trigger"]}
execute store result score @s range_starlit run data get entity @p SelectedItem.components."minecraft:enchantments".center_calculation:range/starlit 3
execute if score @s range_starlit matches 1.. run tag @n[type=armor_stand,tag=arrow_trigger] add starlit_as
execute store result score @s stdTemp1 run function center_calculation:damage/range/calculation
