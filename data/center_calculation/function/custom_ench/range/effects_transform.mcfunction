...
execute at @s on attacker as @s if entity @s[distance=..4] if items entity @s weapon.mainhand *[enchantments~[{enchantments:"center_calculation:range/point_blank"}]] run tag @n[type=!marker] add tempDistance
execute at @s on attacker as @s if entity @s[distance=5..] if items entity @s weapon.mainhand *[enchantments~[{enchantments:"center_calculation:range/sniper"}]] run tag @n[type=!marker] add tempDistance

execute as @s at @s run function center_calculation:custom_ench/range/apply with entity @n[type=marker,tag=temp_effects_transformer] data
