summon marker ~ ~ ~ {Tags:["temp_effects_transformer"],data:{greed:0.0d,freeze:0,ring:0.0f,starlit:0.0f}}
...
execute store result entity @n[type=marker,tag=temp_effects_transformer] data.starlit float 1.0 run data get entity @s active_effects[{id:"minecraft:unluck"}].amplifier 3
...
execute if score @s arrow_apply_greed matches 1.. run effect give @n[type=!marker] glowing 2 0 true
