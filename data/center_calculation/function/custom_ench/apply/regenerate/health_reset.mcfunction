advancement revoke @s only center_calculation:reg_end
execute if predicate {condition:"entity_properties",entity:"this",predicate:{effects:{instant_health:{}}}} run return run function center_calculation:custom_ench/assistance/regeneration
#hfp1-6 hfp1p-hfp6p
attribute @s max_health modifier remove hfp1
attribute @s max_health modifier remove hfp2
attribute @s max_health modifier remove hfp3
attribute @s max_health modifier remove hfp4
attribute @s max_health modifier remove hfp5
attribute @s max_health modifier remove hfp6
attribute @s max_health modifier remove hfp1p
attribute @s max_health modifier remove hfp2p
attribute @s max_health modifier remove hfp3p
attribute @s max_health modifier remove hfp4p
attribute @s max_health modifier remove hfp5p
attribute @s max_health modifier remove hfp6p
attribute @s max_health modifier remove health_boost
attribute @s max_health modifier remove temp_hp_pre
attribute @s max_health base set 20