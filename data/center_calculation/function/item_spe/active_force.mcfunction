...
execute as @a if score @s ChestModel.cd matches 0 if items entity @s hotbar.* *[item_model="minecraft:chest",custom_data~{ChestModel:1b}] at @s run function center_calculation:item_spe/active/chest_model/refresh