...
execute store result score @s magic_protection_p6 run data get entity @s equipment.mainhand.components."minecraft:enchantments"."core:defence/magic_protection" 6
...
tellraw @s [{translate:name.magic_defence.lore,"fallback":"Magic Damage Defence : ",color:white,bold:true,hover_event:{action:"show_text",value:{translate:name.defence.magic.lore,color:white,bold:true}}},{score:{name:"@s",objective:"temp_100"},color:"#ade6ff",bold:true},{"text":"%",color:"#ade6ff",bold:true}]