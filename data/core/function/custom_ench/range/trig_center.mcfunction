...
execute store result score @s range_greed run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/greed"

execute store result score @s range_freeze run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/freeze"

execute store result score @s range_ring run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/ring"

execute store result score @s range_starlit run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/starlit"

execute store result score @s range_sniper_blank run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/sniper"

execute if score @s range_sniper_blank matches 0 store result score @s range_sniper_blank run data get entity @s SelectedItem.components."minecraft:enchantments"."core:range/point_blank"
...
#---------------------transform_effect------------------#
execute if items entity @s weapon.* *[enchantments~[{enchantments:"core:range/transformation",levels:1}]] at @s as @e[type=#minecraft:arrows,distance=..2.5,nbt=!{pickup:0b}] run data merge entity @s {Glowing:1b,SoundEvent:"entity.arrow.hit_player"}
execute if items entity @s weapon.* *[enchantments~[{enchantments:"core:range/transformation",levels:1}]] at @s as @e[type=#minecraft:arrows,distance=..2.5,nbt=!{pickup:0b}] run team join aqua @s
#---------------------transform_effect_end----------------#

#--------------modified_arrow_damage----------------------#
execute as @e[type=#minecraft:arrows,distance=..2.5,nbt=!{pickup:0b}] store result entity @s damage double 0.000028 as @p run function core:damage/range/base
