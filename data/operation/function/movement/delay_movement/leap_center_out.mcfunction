summon armor_stand ~ ~ ~ {Invisible:1b,Invulnerable:1b,Small:1b,Motion:[0.0,0.5,0.0],Tags:["delay.leap_center.out.carrier","timing_limited","timing_limited_10"]}
...
execute at @s as @n[type=armor_stand,tag=delay.leap_center.out.carrier] store result entity @s Motion[0] double 0.008 run scoreboard players operation @s entity_x_det -= @s entity_x
execute at @s as @n[type=armor_stand,tag=delay.leap_center.out.carrier] store result entity @s Motion[2] double 0.008 run scoreboard players operation @s entity_z_det -= @s entity_z
